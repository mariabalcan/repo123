...
int ***paste(int ***image_dst, int N_dst, int M_dst, int *** image_src, int N_src, int M_src, int x, int y) {
	printf("1\n");
	int ***m=alocare(N_dst,M_dst);
	printf("2\n");
	 for (int i = 0; i < N_dst; i++) {
        for (int j = 0; j < M_dst; j++) {
            for (int k = 0; k < 3; k++) {
                m[i][j][k] = image_dst[i][j][k];
            }
        }
    }
		...
		for(int j = 0; j < M; j++)
		{
			...
			for(int i1 = i - filter_size / 2; i1 <= i + filter_size / 2; i1++)
			{
				for(int j1 = j - filter_size / 2; j1 <= j + filter_size / 2; j1++)
				{
					...
					r += image[i1][j1][0] * filter[i2][j2];
					g += image[i1][j1][1] * filter[i2][j2];
					b += image[i1][j1][2] * filter[i2][j2];
				}
			}
